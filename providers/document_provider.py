...
from models.Profile_KYC.document_upload import DocumentType
...
class HyperVergeDocumentProvider:
    ...
    @staticmethod
    def verify(document_type: DocumentType, file_path: str, registered_name: str) -> dict:
        if not settings.HYPERVERGE_APP_ID or not settings.HYPERVERGE_APP_KEY:
            raise ValueError(
        ...
        url = settings.HYPERVERGE_API_URL.rstrip("/") + endpoint
        ...
        try:
            with open(file_path, "rb") as f:
                resp = requests.post(
                    ...
                    headers={"appId": settings.HYPERVERGE_APP_ID, "appKey": settings.HYPERVERGE_APP_KEY},
                    timeout=30,
                )
