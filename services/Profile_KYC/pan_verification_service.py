    ...
    def verify_pan(db: Session, user_id: int) -> dict:
        ...
        if tracker.locked_until:
            ...
            if locked_until > now:
                raise HTTPException(
                    423,
                    f"PAN verification blocked due to {settings.PAN_MAX_ATTEMPTS} failed attempts. "
                    f"Try again after {settings.PAN_COOLDOWN_HOURS} hours.",
                )

            AttemptTrackerRepository.reset_attempts(db, tracker)

        current_attempt = AttemptTrackerRepository.increment_attempt(db, tracker)
        logger.info(f"PAN verification attempt {current_attempt}/{settings.PAN_MAX_ATTEMPTS} for user_id={user_id}")

        if current_attempt > settings.PAN_MAX_ATTEMPTS:
            AttemptTrackerRepository.lock_tracker(
                db, tracker, now + timedelta(hours=settings.PAN_COOLDOWN_HOURS)
            )
            raise HTTPException(
                423,
        ...
        if not result["success"]:
            if current_attempt >= settings.PAN_MAX_ATTEMPTS:
                ...
                msg = f"Maximum attempts reached. Blocked for {settings.PAN_COOLDOWN_HOURS} hours."
            else:
                status = "FAILED"
