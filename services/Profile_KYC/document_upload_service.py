...
class DocumentUploadService:
    ...
    @staticmethod
    def bulk_upload_documents(db: Session, user_id: int, document_types: list, files: list) -> dict:
        if len(files) != len(document_types):
            raise HTTPException(400, "Number of files must match number of document_types")
 
        if len(files) > 4:
            raise HTTPException(400, "Maximum 4 documents allowed per request")
        ...
        results = []
        for document_type, file in zip(document_types, files):
            try:
                try:
                    ...
                    results.append({
                        ...
                        "success":       False,
                        "message":       f"Invalid document type '{document_type}'. Valid: {valid}",
                    })
                    continue
                ...
                if existing_doc and existing_doc.status in [DocumentStatus.VERIFIED, DocumentStatus.APPROVED]:
                    results.append({
                        ...
                        "success":       False,
                        "message":       f"{document_type} already {existing_doc.status.value.lower()}",
                    })
                    continue
                ...
                logger.info(f"Bulk upload: {doc.document_type.value} for user_id={user_id}")
                ...
                logger.error(f"Bulk upload error for {document_type}: {e}", exc_info=True)
                results.append({
                    "document_type": document_type,
                    "file_name":     file.filename if file else "unknown",
                    "success":       False,
        ...
        return {
            ...
            "total_failed":    len(files) - len(success_results),
            "results":         results,
        }
    ...
    def verify_document_background(document_id: int):
        ...
        try:
            ...
            if result["success"]:
                ...
                doc.verification_remarks  = None
                logger.info(f"[BG VERIFY] Document {doc.id} VERIFIED (mode={settings.VERIFICATION_MODE})")
            else:
    ...
    def _validate_file(file: UploadFile, doc_type: DocumentType):
        ...
        if doc_type in [DocumentType.AADHAAR_FRONT, DocumentType.AADHAAR_BACK, DocumentType.PAN_CARD]:
            if file_ext not in settings.ALLOWED_IMAGE_EXTENSIONS:
                raise HTTPException(400, f"Invalid image format. Allowed: {', '.join(settings.ALLOWED_IMAGE_EXTENSIONS)}")
            ...
            if file_ext not in settings.ALLOWED_DOCUMENT_EXTENSIONS:
                raise HTTPException(400, f"Invalid document format. Allowed: {', '.join(settings.ALLOWED_DOCUMENT_EXTENSIONS)}")
