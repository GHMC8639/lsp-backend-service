...
from core.validators import validate_password,validate_mobile_number
...
router = APIRouter(tags=["SuperAdmin | Support Team"])
...
def create_support_user(
    ...
    mobile = validate_mobile_number(payload.mobile)
    ...
    return {
        ...
        "user": {
            ...
            "mobile": support_user.mobile_number,
...
def get_all_support_users(
    ...
    users = db.query(User).filter(User.role == ROLE_SUPPORT).all()
...
def get_support_user_by_mobile(
    ...
    if not user:
        raise HTTPException(404, "Support user not found")
...
def update_support_user(
    ...
    if "password" in update_data:
        validate_password_length(update_data["password"])
        user.password_hash = hash_text(update_data.pop("password"))

    # Update mobile
    if "mobile" in update_data:
        new_mobile = validate_mobile(update_data["mobile"])
...
def delete_support_user(
    ...
    return {"message": "Support user deleted successfully"}